...
| Data | Resumo | Commits |
|---|---|---|
| 27/08/2026 | WP-1.4 (proposta de compensação, resolve Q2) e WP-1.5 (relógio injetável + identidade como parâmetro) concluídos. Corrigida no board a divergência de WP-1.2/WP-1.3 (já commitados desde 26/08, board dizia "não commitado"). Achado durante verificação: `window.explodir`/`window.matDaOperacao` quebram em runtime com `PARAM.baixaAutomatica` no padrão (bug pré-existente do WP-1.1, não corrigido — ver Achados). E1 completa nos WPs executáveis; falta ratificação de Q2 para abrir F-2.1/E2. | a commitar nesta sessão |
