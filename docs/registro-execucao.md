...
**Etapa corrente:** E1 — Rede de proteção de domínio (WP-1.1 a WP-1.5). WP-1.1 CONCLUIDO e commitado (`318659f`); WP-1.2 e WP-1.3 CONCLUIDOS, não commitados; WP-1.4 e WP-1.5 PENDENTES. Trabalho de implementação decorrente de P-13/O2 (edição da SPEC §4.6) e P-14/O1 (correção de `grupoChaveOS()` + decisão sobre migrar/invalidar `DB.os_ids`) permanece não executado — fora do escopo desta sequência de WPs, aguardando confirmação do dono para iniciar.
