...
**Última sessão:** 27/08/2026 — WP-1.4 (design da compensação, resolve Q2) e WP-1.5 (relógio injetável + identidade como parâmetro) concluídos e commitados. Corrigida divergência do board: WP-1.2/WP-1.3 já estavam commitados desde 26/08 (`d5eb4b5`, `8358507`) mas a linha ainda dizia "não commitado". **E1 completa nos 5 WPs de implementação; falta só a ratificação de Q2 pelo dono do produto antes de abrir F-2.1.**
**Etapa corrente:** E1 concluída na parte executável pelo agente (WP-1.1 a WP-1.5, todos CONCLUIDOS e commitados). **Bloqueio para abrir E2:** Q2 (forma da compensação) tem proposta escrita em `docs/wp-1.4-design-compensacao-20260827.md`, aguardando ratificação explícita do dono do produto — gate declarado no próprio plano (§7). Trabalho de implementação decorrente de P-13/O2 (edição da SPEC §4.6) e P-14/O1 (correção de `grupoChaveOS()` + decisão sobre migrar/invalidar `DB.os_ids`) permanece não executado — fora do escopo desta sequência de WPs, aguardando confirmação do dono para iniciar.
...
- 27/08 — **Bug ativo pré-existente (WP-1.1), achado durante verificação em navegador do WP-1.5: `window.explodir` e `window.matDaOperacao` lançam `TypeError: window.ficha is not a function` sempre que chamados com interação real.** Causa: os wrappers reexpostos em `prototype/index.html:26090-26097` (`<script type="module">`, WP-1.1) injetam `ficha`/`comp`/`item` como `window.ficha`, `window.comp`, `window.item` — mas as três são `const` no legado (`ficha` em `:19347`, `comp` em `:19345`, `item` em `:16416`), não `function`; declaração `const` no escopo global de um `<script>` clássico não vira propriedade de `window` (ao contrário de `function`, que `custoMat`/`custoProc`/`custoMO` usam corretamente — por isso `window.custoAberto` funciona). Agravante em `item`: existe `<section id="item">` no HTML, então `window.item` resolve por *named access* para esse elemento DOM (`typeof window.item === 'object'`), então mesmo convertendo `item` para `function` o wrapper continuaria errado enquanto chamado via `window.item` — precisa fechar sobre o identificador léxico `item`, não sobre `window.item`. `window.comp` e `window.ficha` são simplesmente `undefined`. Alcance: qualquer `finalizarOperacao` com `PARAM.baixaAutomatica === true` (**valor padrão do seed**, `:15346`) quebra antes de abrir o modal de confirmação (`:16921`); `baixaAutomatica()` (`:21546`) e a segunda chamada em `finalizarOperacao` (`:17014`) têm o mesmo problema. Reproduzido em navegador real (Chrome via harness, HTTP local, perfil Operador, OP-2431): com `PARAM.baixaAutomatica` no padrão, clicar "Finalizar operação" lança a exceção e a operação nunca finaliza; com `PARAM.baixaAutomatica = false` (contornado só para verificar o WP-1.5, não uma correção), o fluxo completa normalmente. Não corrigido — fora do escopo de WP-1.5; bloqueia F-2.1 (que converte exatamente esta cadeia) se não for resolvido antes.
...
| Data | Resumo | Commits |
|---|---|---|
| 27/08/2026 | WP-1.4 (proposta de compensação, resolve Q2) e WP-1.5 (relógio injetável + identidade como parâmetro) concluídos. Corrigida no board a divergência de WP-1.2/WP-1.3 (já commitados desde 26/08, board dizia "não commitado"). Achado durante verificação: `window.explodir`/`window.matDaOperacao` quebram em runtime com `PARAM.baixaAutomatica` no padrão (bug pré-existente do WP-1.1, não corrigido — ver Achados). E1 completa nos WPs executáveis; falta ratificação de Q2 para abrir F-2.1/E2. | `e74bf95` |
