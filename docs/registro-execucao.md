...
**Última sessão:** 25/08/2026 (noite) — WP-0.3 concluído: os 18 itens do checklist §6.1/§6.2 da SPEC0824 foram executados em navegador real e a ata `docs/verificacao-p0-20260824.md` foi escrita. 17 passaram, 1 reprovou (RNF-03, achado A-02); achado adjacente A-03 registrado.
**Etapa corrente:** E0 **encerrada no que depende do agente** (WP-0.1 a WP-0.6). Bloqueio único para entrar na E1: **o commit inicial do WP-0.1 é ação do dono do produto (P-01)**; sem ele não há linha de base para edição de código — a correção de A-01 e as correções de A-02/A-03 estão todas sem rede de proteção. Duas decisões novas aguardam o dono: **P-13 (A-02)** e **P-14 (A-03)**.
...
| WP | Entrega | Estado |
|---|---|---|
| WP-0.1 | `git init` + `.gitignore` + commit inicial | **BLOQUEADO(commit inicial é do dono)** — `git init` executado (`.git/` criado, `git status` responde); `.gitignore` criado cobrindo `node_modules/`, `.env*`, `supabase/.temp/`, `supabase/.branches/`, previews gerados; `git status --short` lista 15 entradas não rastreadas, nenhum segredo entre elas. Falta só o commit inicial, que é ação do dono do produto (regra permanente: agente não commita). |
...
| WP | Entrega | Estado |
|---|---|---|
| WP-1.1 | Regras quase-puras em `prototype/core/*.mjs` + caracterização | PENDENTE |
...
|---|---|---|---|
| P-01 | **Commit inicial** do WP-0.1 (`git init` e `.gitignore` já feitos pelo agente; commit é ação exclusiva do dono, por regra permanente) | Toda edição de código da E1 em diante — sem linha de base não há rede de proteção (risco R1) | `git add -A` + `git commit` na raiz do repositório |
...
| P-13 | **A-02 — RNF-03 reprovado: publicar ficha não usa o contrato de `confirmarEfeito()`** (sem tabela de efeito; nota de perigo condicional). Único item reprovado no checklist §6.2 | O critério "zero P0 remanescentes verificados" do PRD, enquanto não resolvido | Escolher: **O1** aplicar o contrato de `confirmarEfeito()` em `#fi-save` (`:20768`), com tabela de efeito e nota `danger` incondicional — muda o conteúdo do modal, **passa pelo design lock**; ou **O2** alterar a SPEC (§4.6 :608) para tirar `fn_publicar_ficha` da lista das 4 mutações críticas, assumindo que o modal atual já é confirmação suficiente. Depende de P-01 nos dois casos |
| P-14 | **A-03 — `os_id` e QR colidem entre folhas de OS distintas** que atendem o mesmo conjunto de ordens | Nada hoje no checklist (RF-40/41 passou pelo texto); trava o uso operacional real do QR em emissão consolidada por setor/máquina | Escolher: **O1** incluir `OS.grupo` + valor do grupo na `grupo_chave` de `grupoChaveOS()` (`:25397`) — os ids já emitidos mudam de significado, exige decisão sobre migrar ou invalidar `DB.os_ids` gravado; ou **O2** aceitar a colisão e tratar a folha como identificada por `os_id` + setor impresso, ajustando o comentário `:25387-25395` para não prometer o que não entrega. **Recomendação técnica: O1** — o QR é lido no chão de fábrica sem contexto |
